...
import type { CreateTeamFormData, EditTeamFormData } from "./teamManagement.schemas";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        <Button data-testid="button-create-team">
          ...
          {t('admin.teamMgmtDialogs.createTeam')}
        </Button>
      </DialogTrigger>
      <DialogContent data-testid="dialog-create-team">
        <DialogHeader>
          <DialogTitle>{t('admin.teamMgmtDialogs.createNewTeam')}</DialogTitle>
          <DialogDescription>
            {t('admin.teamMgmtDialogs.createTeamDesc')}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('admin.teamMgmtDialogs.teamName')}</FormLabel>
                  <FormControl>
                    <Input placeholder={t('admin.teamMgmtDialogs.teamNamePlaceholder')} data-testid="input-team-name" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('admin.teamMgmtDialogs.description')}</FormLabel>
                  <FormControl>
                    <Input placeholder={t('admin.teamMgmtDialogs.descriptionPlaceholder')} data-testid="input-team-description" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('admin.teamMgmtDialogs.colorLabel')}</FormLabel>
                  <FormControl>
            ...
            <DialogFooter>
              <Button type="submit" data-testid="button-submit-create-team" disabled={isPending}>
                {isPending ? t('admin.teamMgmtDialogs.creating') : t('admin.teamMgmtDialogs.createTeam')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-testid="dialog-edit-team">
        <DialogHeader>
          <DialogTitle>{t('admin.teamMgmtDialogs.editTeam')}</DialogTitle>
          <DialogDescription>
            {t('admin.teamMgmtDialogs.updateTeamInfo')}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              <Button type="submit" data-testid="button-submit-edit-team" disabled={isPending}>
                {isPending ? t('admin.teamMgmtDialogs.updating') : t('admin.teamMgmtDialogs.updateTeam')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl" data-testid="dialog-team-members">
        <DialogHeader>
          <DialogTitle>{t('admin.teamMgmtDialogs.teamMembersTitle', { name: selectedTeam?.name })}</DialogTitle>
          <DialogDescription>
            {t('admin.teamMgmtDialogs.manageMembers')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              >
                <SelectTrigger className="flex-1" data-testid="select-add-member">
                  <SelectValue placeholder={t('admin.teamMgmtDialogs.selectUserToAdd')} />
                </SelectTrigger>
                <SelectContent>
          ...
          {/* Members List */}
          {membersLoading ? (
            <div className="text-center py-8 text-muted-foreground">{t('admin.teamMgmtDialogs.loadingMembers')}</div>
          ) : members.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {t('admin.teamMgmtDialogs.noMembers')}
            </div>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('admin.teamMgmtDialogs.name')}</TableHead>
                    <TableHead>{t('admin.teamMgmtDialogs.email')}</TableHead>
                    <TableHead>{t('admin.teamMgmtDialogs.roleInTeam')}</TableHead>
                    <TableHead>{t('admin.teamMgmtDialogs.joined')}</TableHead>
                    <TableHead className="text-right">{t('admin.teamMgmtDialogs.actions')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {members.map((member) => (
                      ...
                      <TableCell className="text-right">
                        <Can permissions={["team:manage"]}>
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              if (selectedTeam && confirm(t('admin.teamMgmtDialogs.confirmRemoveMember'))) {
