  ...
  async generateArtifactDraft(params: {
    ...
    try {
      ...
      if (forceClaude) {
        ...
        // REQUIREMENTS / BUSINESS_CASE / STRATEGIC_FIT drafts are large, deeply
        // structured artifacts. Force a generous max_tokens ceiling so the
        // response is not truncated mid-JSON, regardless of DB engine config.
        const largeArtifactCfg = {
          ...engineCfg,
          maxTokens: Math.max(engineCfg.maxTokens ?? 0, 20000),
        };
        ...
        let finalRaw = raw;
        logger.info(
          `[Hybrid Engine][DEBUG] ${artifactType} raw length=${(raw || "").length} parsedKeys=${parsed ? Object.keys(parsed).length : 0} topKeys=${parsed ? Object.keys(parsed).slice(0, 20).join(",") : "null"}`
        );
  ...
  private parseJsonObject(text: string): Record<string, unknown> | null {
    ...
    if (allBalanced.length > 0) {
      ...
      if (parsedCandidates.length > 0) {
        // Prefer the artifact wrapper (has artifactType) over any nested sub-object.
        const withArtifactType = parsedCandidates.filter((c) => typeof c.artifactType === "string");
        const pool = withArtifactType.length > 0 ? withArtifactType : parsedCandidates;
        pool.sort((a, b) => Object.keys(b).length - Object.keys(a).length);
        return pool[0] ?? null;
      }
    }

    // Fallback 1: try to parse the entire cleaned string directly.
    const direct = tryParseObject(cleaned);
    if (direct) return direct;

    // Fallback 2: response was likely truncated mid-string. Attempt a repair
    // by taking the first '{', closing any open string, and then auto-closing
    // every still-open brace / bracket in LIFO order. This turns a truncated
    // artifact into valid-enough JSON that preserves whatever was emitted.
    ...
    if (firstBrace >= 0) {
      ...
      let esc = false;
      let lastSafe = 0; // last byte index that is NOT inside an unterminated string
      for (let i = 0; i < body.length; i += 1) {
        const ch = body[i] ?? "";
        if (esc) { esc = false; continue; }
        if (inStr) {
          if (ch === "\\") { esc = true; continue; }
          if (strQuote && ch === strQuote) { inStr = false; strQuote = null; lastSafe = i; }
          continue;
        }
        if (ch === '"' || ch === "'") { inStr = true; strQuote = ch as '"' | "'"; continue; }
        if (ch === "{" || ch === "[") { stack.push(ch); lastSafe = i; continue; }
        if (ch === "}" || ch === "]") { stack.pop(); lastSafe = i; continue; }
        if (ch === "," || ch === ":" || /\s/.test(ch)) { lastSafe = i; continue; }
        lastSafe = i;
      }
      // Truncate at the last safe (non-in-string) boundary and strip any trailing
      // partial token (e.g. an incomplete key like `"security` that has no colon yet).
      let trimmed = body.slice(0, lastSafe + 1);
      // Drop trailing partial `, "key` or `, "key":` without value.
      ...
      const repaired = tryParseObject(trimmed);
      if (repaired) {
        return repaired;
      }
    }
