...
export class IntelligencePlanBuilder {
  ...
  async build(ctx: IPlanBuildContext): Promise<IntelligencePlan> {
    ...
    const redactionMode = this.resolveRedactionMode(routing, ctx.classification);

    // 6. Determine mode (READ vs PLAN)

    // 7. Set budgets
    const budgets = this.resolveBudgets(ctx.useCaseType, routing.primaryEngineKind);

    // 8. Determine allowed tools
    const toolsAllowed = this.resolveTools(routing, ctx.useCaseType);
    ...
    logger.info(
      `[IPLAN] Built: ${iplanId} | Engine: ${routing.primaryEngineKind} | Plugin: ${effectiveSelection.plugin?.name || "NONE"} | Redaction: ${redactionMode} | HITL: ${routing.requiresHITL}`
    );
  ...
  private resolveTools(routing: RoutingDecision, useCaseType: UseCaseType): string[] {
    const tools: string[] = ["rag_search", "entity_extraction", "scoring"];

    if (routing.primaryEngineKind === "EXTERNAL_HYBRID") {
