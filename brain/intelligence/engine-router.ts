 ...
 *
 * SOVEREIGN / HIGH_SENSITIVE → Engine A ONLY (never external, sovereign boundary)
 * CONFIDENTIAL              → Engine A ONLY (external LLMs blocked by classification)
 * INTERNAL                  → Engine B primary with masking; Engine A fallback
 * PUBLIC                    → Engine B primary (no masking needed); Engine A fallback
 */
...
> = {
  ...
  INTERNAL: {
    primaryKind: "EXTERNAL_HYBRID",    // Engine B primary — classification allows external
    fallbackKind: "SOVEREIGN_INTERNAL", // Engine A fallback if external providers down
    allowExternal: true,
    requiresRedaction: true,           // redact PII for external
    requiresHITL: false,
    localOnly: false,
  },
  PUBLIC: {
    primaryKind: "EXTERNAL_HYBRID",    // Engine B primary — public data, no restrictions
    fallbackKind: "SOVEREIGN_INTERNAL", // Engine A fallback if external providers down
    allowExternal: true,
