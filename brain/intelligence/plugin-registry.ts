...
const CAPABILITY_ALIASES: Record<string, PluginCapability> = {
  generateBusinessCase: "BUSINESS_CASE",
  businessCase: "BUSINESS_CASE",
  generateRequirements: "REQUIREMENTS",
  requirements: "REQUIREMENTS",
  generateWBS: "WBS",
  wbs: "WBS",
  generateStrategicFit: "STRATEGIC_FIT",
  ...
  trainingDataGeneration: "TRAINING_DATA",
  rag: "GENERAL_REASONING",
  scoring: "GENERAL_REASONING",
  entities: "GENERAL_REASONING",
  patterns: "GENERAL_REASONING",
  localInference: "GENERAL_REASONING",
  score: "GENERAL_REASONING",
};

function normalizeCapability(value: string): PluginCapability | null {
  const direct = value.toUpperCase() as PluginCapability;
  if ([
    "BUSINESS_CASE", "REQUIREMENTS", "WBS", "STRATEGIC_FIT", "RISK_ANALYSIS",
    "GENERAL_REASONING", "SUMMARIZATION", "CLASSIFICATION", "DISTILLATION",
    "PATTERN_EXTRACTION", "TRAINING_DATA",
  ].includes(direct)) {
    return direct;
  }
    ...
    try {
      ...
      for (const dbPlugin of dbPlugins) {
        ...
        if (Array.isArray(caps)) {
          capabilities = caps
            .map((cap) => typeof cap === "string" ? normalizeCapability(cap) : null)
            .filter((cap): cap is PluginCapability => !!cap);
        } else if (caps && typeof caps === "object") {
          capabilities = Object.entries(caps)
            .filter(([, v]) => v === true)
            .map(([k]) => normalizeCapability(k))
            .filter((cap): cap is PluginCapability => !!cap);
        }
    ...
    if (criteria.requiredCapability) {
      ...
      if (capFiltered.length > 0) {
        ...
        return {
          plugin: null,
          fallbackChain: [],
          reason: `No plugins for role ${criteria.kind} support required capability ${criteria.requiredCapability}`,
        };
      }
    }
