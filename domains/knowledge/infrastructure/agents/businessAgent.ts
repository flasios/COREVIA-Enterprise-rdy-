...
export class BusinessAgent extends BaseRAGAgent {
  domain = 'business';
  
  config: DomainConfig = {
    domain: 'business',
    keywords: ['strategic', 'business value', 'stakeholder', 'UAE Vision 2071', 'transformation', 'innovation', 'citizen', 'service', 'impact', 'alignment'],
    fallbackTerms: ['goal', 'objective', 'outcome', 'benefit', 'value proposition'],
    minScore: 0.6,
    rerankBoost: 1.25,
    topK: 8
  };
  ...
  protected extractDomainInsights(response: string): Record<string, any> | undefined {
    ...
    // UAE Vision 2071 pillars
    const pillars = ['Education', 'Economy', 'Government Development', 'Cohesive Society'];
    insights.visionPillars = pillars.filter(p => response.includes(p));
    
    // Strategic alignment score mention
    const alignmentMatch = /alignment.*?(\d+)\s*%/i.exec(response);
    if (alignmentMatch?.[1]) insights.alignmentScore = Number.parseInt(alignmentMatch[1], 10);
