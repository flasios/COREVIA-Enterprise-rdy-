...
export class TechnicalAgent extends BaseRAGAgent {
  ...
  protected extractDomainInsights(response: string): Record<string, any> | undefined {
    ...
    const techPatterns = ['React', 'Node.js', 'PostgreSQL', 'AWS', 'Azure', 'Kubernetes', 'Docker'];
    insights.techStack = techPatterns.filter(tech => response.includes(tech));
    
    // Scalability mentions
    if (response.toLowerCase().includes('highly scalable')) insights.scalability = 'high';
    if (response.toLowerCase().includes('horizontally scalable')) insights.scalability = 'horizontal';
